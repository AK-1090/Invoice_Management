...
router.put("/:id", async (req, res) => {
  try {
    // Set timeout for the operation to prevent hanging
    const updatePromise = Invoice.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });

    // Add timeout: reject promise after 10 seconds
...
router.get("/:id/pdf", async (req, res) => {
  try {
    ...
    console.log(
      `Generating PDF for invoice ${invoiceId} with template ${template}`,
    );
